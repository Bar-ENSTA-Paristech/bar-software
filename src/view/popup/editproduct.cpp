...
{
    catPrdVector = controller->getProductsCategories();
    ...
    categorie->clear();
    tva->clear();
    int n = catPrdVector.size();
    for(int i = 0 ; i < n ; i++)
    {
        catTuple = catPrdVector[i];
...
void EditProduct::categorieSelected(int index)
{
    ...
    name->clear();
    isPression = QString::fromStdString(catPrdVector[index].getCategoryName()).toLower()== "pression";
    if(isPression)
